 ...
 *  - 별도의 객체생성(new) 하지않고 해당 클래스 타입으로 참조, 실행
...
class Increment {
	private int num1;
	private static int num2; // static 선언은 변수를 class area에 저장되어 모든 명령에 공유한다.
...
public class StaticTest {
	public static void main(String[] args) {
		...
		Increment.show(); //num2는 class area에 저장되어 있어 show를 직접 호출해도 무관하다.
		...
		// 싱글톤(Singleton) 객체 실습
		Calc c1 = Calc.getInstance(); // 생성자가 아닌 클래스 에서 불러온다.
